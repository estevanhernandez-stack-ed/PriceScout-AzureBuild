...
def login(username, password):
    ...
    try:
        response = requests.post(f"{BASE_URL}/login/token", data={"username": username, "password": password}, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def get_all_markets(token: str):
    ...
    try:
        ...
        response = requests.get(f"{BASE_URL}/markets", headers=headers, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def get_markets_by_company(token: str, company: str):
    ...
    try:
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.get(f"{BASE_URL}/markets", headers=headers, params={"company": company}, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def get_theaters_by_market(company: str, market: str):
    ...
    try:
        response = requests.get(f"{BASE_URL}/theaters", params={"company": company, "market": market}, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def create_scrape_run(token: str, mode: str, context: str):
    ...
    try:
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.post(f"{BASE_URL}/scrape_runs", headers=headers, json={"mode": mode, "context": context}, timeout=10)
        response.raise_for_status()
...
def change_password(token: str, old_password: str, new_password: str):
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/users/change-password", headers=headers, json=data, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def reset_password_request(username: str):
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/users/reset-password-request", json=data, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def reset_password_with_code(username: str, code: str, new_password: str):
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/users/reset-password-with-code", json=data, timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
